 ...
 *                http://www.opensource.org/licenses/mit-license.php
  * !!! Modified by Yury Kuchura to support pixel drawing callback for 8BPP instead of writing to allocated memory
 */
...
static inline int8_t read_int8(const uint8_t *data, unsigned int o)
{
    return (int8_t) data[o];
}

static inline uint8_t read_uint8(const uint8_t *data, unsigned int o)
{
    return (uint8_t) data[o];
}

static inline int16_t read_int16(const uint8_t *data, unsigned int o)
{
    return (int16_t)(data[o] | (data[o + 1] << 8));
}

static inline uint16_t read_uint16(const uint8_t *data, unsigned int o)
{
    return (uint16_t)(data[o] | (data[o + 1] << 8));
}

static inline int32_t read_int32(const uint8_t *data, unsigned int o)
{
    return (int32_t)(data[o] | (data[o + 1] << 8) | (data[o + 2] << 16) | (data[o + 3] << 24));
}

static inline uint32_t read_uint32(const uint8_t *data, unsigned int o)
{
...
//      return BMP_DATA_ERROR;
    ...
    for(i = 0; i < count; i++)
    {
        ...
        image->bmp.bmp_data = (uint8_t *)ico->ico_data + read_uint32(data, 12);
        image->bmp.ico = true;
...
static bmp_result bmp_decode_rgb(bmp_image *bmp, uint8_t **start, int bytes)
{
    uint8_t *top, *end, *data;
    //uint8_t *bottom; // - not used
    //uint32_t *scanline;
    intptr_t addr;
    uint32_t x, y;
    //uint32_t swidth; // - not used
    ...
    for(y = 0; y < bmp->height; y++)
    {
        ...
        for(x = 0; x < bmp->width; x++)
        {
            ...
            if(bmp->reversed)
            {
                bmp->bitmap_callbacks.bitmap_put_color(bmp->colour_table[(cur_byte >> bit_shifts[bit++]) & bit_mask], x, y);
            }
            else
            {
                bmp->bitmap_callbacks.bitmap_put_color(bmp->colour_table[(cur_byte >> bit_shifts[bit++]) & bit_mask], x, bmp->height - y - 1);
            }
            //if((bmp->limited_trans) && (scanline[x] == bmp->transparent_index))
            //{
                ////scanline[x] = bmp->trans_colour;
                //bmp->bitmap_callbacks.bitmap_put_color(bmp->trans_colour, x, y);
            //}
        }
    }
    *start = data;
